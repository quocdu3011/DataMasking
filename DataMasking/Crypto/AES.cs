using System;

namespace DataMasking.Crypto
{
    // Triển khai AES-256 từ đầu
    public class AES
    {
        // S-Box cho AES
        private static readonly byte[] SBox = {
            
        };

        // Inverse S-Box
        private static readonly byte[] InvSBox = {
            
        };

        // Rcon cho key expansion
        private static readonly byte[] Rcon = {
            0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
        };

        private byte[,] state = new byte[4, 4];
        private byte[] key;
        private byte[,] roundKeys;
        private int Nr; // Số vòng

        public AES(byte[] key)
        {
            if (key.Length != 16 && key.Length != 24 && key.Length != 32)
                throw new ArgumentException("Key phải là 128, 192 hoặc 256 bit");

            this.key = key;
            int Nk = key.Length / 4;
            Nr = Nk + 6;
            
            KeyExpansion(Nk);
        }

        // Mở rộng khóa
        private void KeyExpansion(int Nk)
        {
            int totalWords = 4 * (Nr + 1);
            roundKeys = new byte[totalWords, 4];

            // Copy khóa gốc
            for (int i = 0; i < Nk; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    roundKeys[i, j] = key[i * 4 + j];
                }
            }

            // Tạo các round key
            for (int i = Nk; i < totalWords; i++)
            {
                byte[] temp = new byte[4];
                for (int j = 0; j < 4; j++)
                    temp[j] = roundKeys[i - 1, j];

                if (i % Nk == 0)
                {
                    temp = SubWord(RotWord(temp));
                    temp[0] ^= Rcon[i / Nk];
                }
                else if (Nk > 6 && i % Nk == 4)
                {
                    temp = SubWord(temp);
                }

                for (int j = 0; j < 4; j++)
                {
                    roundKeys[i, j] = (byte)(roundKeys[i - Nk, j] ^ temp[j]);
                }
            }
        }

        private byte[] RotWord(byte[] word)
        {
            return new byte[] { word[1], word[2], word[3], word[0] };
        }

        private byte[] SubWord(byte[] word)
        {
            for (int i = 0; i < 4; i++)
                word[i] = SBox[word[i]];
            return word;
        }

        // Mã hóa 1 block (16 bytes)
        public byte[] EncryptBlock(byte[] input)
        {
            if (input.Length != 16)
                throw new ArgumentException("Input phải là 16 bytes");

            // Copy input vào state
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    state[j, i] = input[i * 4 + j];

            AddRoundKey(0);

            for (int round = 1; round < Nr; round++)
            {
                SubBytes();
                ShiftRows();
                MixColumns();
                AddRoundKey(round);
            }

            SubBytes();
            ShiftRows();
            AddRoundKey(Nr);

            // Copy state ra output
            byte[] output = new byte[16];
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    output[i * 4 + j] = state[j, i];

            return output;
        }

        // Giải mã 1 block
        public byte[] DecryptBlock(byte[] input)
        {
            if (input.Length != 16)
                throw new ArgumentException("Input phải là 16 bytes");

            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    state[j, i] = input[i * 4 + j];

            AddRoundKey(Nr);

            for (int round = Nr - 1; round > 0; round--)
            {
                InvShiftRows();
                InvSubBytes();
                AddRoundKey(round);
                InvMixColumns();
            }

            InvShiftRows();
            InvSubBytes();
            AddRoundKey(0);

            byte[] output = new byte[16];
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    output[i * 4 + j] = state[j, i];

            return output;
        }

        private void SubBytes()
        {
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    state[i, j] = SBox[state[i, j]];
        }

        private void InvSubBytes()
        {
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    state[i, j] = InvSBox[state[i, j]];
        }

        private void ShiftRows()
        {
            byte temp;
            
            // Row 1: shift 1
            temp = state[1, 0];
            state[1, 0] = state[1, 1];
            state[1, 1] = state[1, 2];
            state[1, 2] = state[1, 3];
            state[1, 3] = temp;

            // Row 2: shift 2
            temp = state[2, 0];
            state[2, 0] = state[2, 2];
            state[2, 2] = temp;
            temp = state[2, 1];
            state[2, 1] = state[2, 3];
            state[2, 3] = temp;

            // Row 3: shift 3
            temp = state[3, 3];
            state[3, 3] = state[3, 2];
            state[3, 2] = state[3, 1];
            state[3, 1] = state[3, 0];
            state[3, 0] = temp;
        }

        private void InvShiftRows()
        {
            byte temp;
            
            // Row 1: shift right 1
            temp = state[1, 3];
            state[1, 3] = state[1, 2];
            state[1, 2] = state[1, 1];
            state[1, 1] = state[1, 0];
            state[1, 0] = temp;

            // Row 2: shift right 2
            temp = state[2, 0];
            state[2, 0] = state[2, 2];
            state[2, 2] = temp;
            temp = state[2, 1];
            state[2, 1] = state[2, 3];
            state[2, 3] = temp;

            // Row 3: shift right 3
            temp = state[3, 0];
            state[3, 0] = state[3, 1];
            state[3, 1] = state[3, 2];
            state[3, 2] = state[3, 3];
            state[3, 3] = temp;
        }

        private byte GMul(byte a, byte b)
        {
            byte p = 0;
            for (int i = 0; i < 8; i++)
            {
                if ((b & 1) != 0)
                    p ^= a;
                
                bool hiBitSet = (a & 0x80) != 0;
                a <<= 1;
                if (hiBitSet)
                    a ^= 0x1B;
                b >>= 1;
            }
            return p;
        }

        private void MixColumns()
        {
            for (int i = 0; i < 4; i++)
            {
                byte s0 = state[0, i];
                byte s1 = state[1, i];
                byte s2 = state[2, i];
                byte s3 = state[3, i];

                state[0, i] = (byte)(GMul(s0, 2) ^ GMul(s1, 3) ^ s2 ^ s3);
                state[1, i] = (byte)(s0 ^ GMul(s1, 2) ^ GMul(s2, 3) ^ s3);
                state[2, i] = (byte)(s0 ^ s1 ^ GMul(s2, 2) ^ GMul(s3, 3));
                state[3, i] = (byte)(GMul(s0, 3) ^ s1 ^ s2 ^ GMul(s3, 2));
            }
        }

        private void InvMixColumns()
        {
            for (int i = 0; i < 4; i++)
            {
                byte s0 = state[0, i];
                byte s1 = state[1, i];
                byte s2 = state[2, i];
                byte s3 = state[3, i];

                state[0, i] = (byte)(GMul(s0, 14) ^ GMul(s1, 11) ^ GMul(s2, 13) ^ GMul(s3, 9));
                state[1, i] = (byte)(GMul(s0, 9) ^ GMul(s1, 14) ^ GMul(s2, 11) ^ GMul(s3, 13));
                state[2, i] = (byte)(GMul(s0, 13) ^ GMul(s1, 9) ^ GMul(s2, 14) ^ GMul(s3, 11));
                state[3, i] = (byte)(GMul(s0, 11) ^ GMul(s1, 13) ^ GMul(s2, 9) ^ GMul(s3, 14));
            }
        }

        private void AddRoundKey(int round)
        {
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    state[j, i] ^= roundKeys[round * 4 + i, j];
                }
            }
        }

        // Mã hóa CBC mode
        public byte[] EncryptCBC(byte[] plaintext, byte[] iv)
        {
            if (iv.Length != 16)
                throw new ArgumentException("IV phải là 16 bytes");

            // Padding PKCS7
            int paddingLength = 16 - (plaintext.Length % 16);
            byte[] paddedData = new byte[plaintext.Length + paddingLength];
            Array.Copy(plaintext, paddedData, plaintext.Length);
            for (int i = plaintext.Length; i < paddedData.Length; i++)
                paddedData[i] = (byte)paddingLength;

            byte[] ciphertext = new byte[paddedData.Length];
            byte[] previousBlock = iv;

            for (int i = 0; i < paddedData.Length; i += 16)
            {
                byte[] block = new byte[16];
                Array.Copy(paddedData, i, block, 0, 16);

                // XOR với block trước
                for (int j = 0; j < 16; j++)
                    block[j] ^= previousBlock[j];

                byte[] encryptedBlock = EncryptBlock(block);
                Array.Copy(encryptedBlock, 0, ciphertext, i, 16);
                previousBlock = encryptedBlock;
            }

            return ciphertext;
        }

        // Giải mã CBC mode
        public byte[] DecryptCBC(byte[] ciphertext, byte[] iv)
        {
            if (iv.Length != 16)
                throw new ArgumentException("IV phải là 16 bytes");

            if (ciphertext.Length % 16 != 0)
                throw new ArgumentException("Ciphertext length phải chia hết cho 16");

            byte[] plaintext = new byte[ciphertext.Length];
            byte[] previousBlock = iv;

            for (int i = 0; i < ciphertext.Length; i += 16)
            {
                byte[] block = new byte[16];
                Array.Copy(ciphertext, i, block, 0, 16);

                byte[] decryptedBlock = DecryptBlock(block);

                // XOR với block trước
                for (int j = 0; j < 16; j++)
                    decryptedBlock[j] ^= previousBlock[j];

                Array.Copy(decryptedBlock, 0, plaintext, i, 16);
                previousBlock = block;
            }

            // Remove padding
            int paddingLength = plaintext[plaintext.Length - 1];
            byte[] result = new byte[plaintext.Length - paddingLength];
            Array.Copy(plaintext, result, result.Length);

            return result;
        }
    }
}
